...
public class CalendarConverter extends AbstractConverter {
    ...
    private <T> T toDate(final Class<T> type, final String value) {
        // java.util.Calendar
        if (type.equals(Calendar.class)) {
            return type.cast(CalendarConverter.valueOf(value));
        }
    ...
    String toString(final Class<?> type) {
        ...
        } else if (typeName.startsWith(CalendarConverter.PACKAGE)) {
            typeName = typeName.substring(CalendarConverter.PACKAGE.length());
        }
        return typeName;
    }
    ...
    public static Calendar valueOf(String s) {
        ...
        long millis = new Timestamp(year - 1900, month - 1, day, hour, minute, second, a_nanos).getTime();
        Calendar cal = Calendar.getInstance();
