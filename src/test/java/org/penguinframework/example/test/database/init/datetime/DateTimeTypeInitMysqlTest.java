...
class DateTimeTypeInitMysqlTest extends DateTimeTypeInitAbstractTest {
    private static final int NANO_SECOND_PRECISION_SCALE = 6;
    private static final BigDecimal ROUND_DOWN_CALC_VALUE = BigDecimal
            .valueOf(Math.pow(10, 9 - DateTimeTypeInitMysqlTest.NANO_SECOND_PRECISION_SCALE));
    ...
    LocalTime toDatabaseScaleTime(LocalTime from) {
        // MYSQLの時刻型で扱える精度はマイクロ秒で四捨五入
        int nano = BigDecimal.valueOf(from.getNano())
                .divide(DateTimeTypeInitMysqlTest.ROUND_DOWN_CALC_VALUE, RoundingMode.HALF_UP)
                .multiply(DateTimeTypeInitMysqlTest.ROUND_DOWN_CALC_VALUE).intValue();
